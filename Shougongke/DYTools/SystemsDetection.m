...
@implementation SystemsDetection


#pragma mark - 检测系统设备型号
//IOS 获取最新设备型号方法 列表最新对照表：https://www.theiphonewiki.com/wiki/Models
//如果出了新设备，可以在上面的链接找到对应的
+ (NSString *)getDeviceVersCoding{
    NSString *platform = [SystemsDetection getDeviceVersionInfo];
    if ([platform isEqualToString:@"iPhone 4"]
        || [platform isEqualToString:@"iPhone 4S"]) {
        return @"480";
    }
    if ([platform isEqualToString:@"iPhone 5"]
        || [platform isEqualToString:@"iPhone 5c"]
        || [platform isEqualToString:@"iPhone 5s"]) {
        return @"568";
    }
